    ...
    for _ in range(population_size):
        ...
        '''for asteroid_id in range(1, n_asteroids + 1):
            ...
            if(opp_elngth > 0 ):  # Ensure there are opportunities available for this asteroid/station
                ...
                assignments.extend([asteroid_id, station_id, opportunity_id])
            else:
...
def swap_mutation(chromosome,  n_asteroids, n_stations, mutation_rate):
    # Calculate time window and asteroid assignment sections
    time_window_length = n_stations * 2  # First 24 elements are time windows
    asteroid_assignments_start = time_window_length  # Remaining elements are asteroid assignments

    
    # Check if mutation should occur
    if random.random() < mutation_rate:
        # Swap mutation for time windows (first 24 elements)
        point1 = random.randint(1, time_window_length - 1)  # Pick random point between 1 and 23
        
        # Perform the swap without changing values, just re-ordering by creating a new list
        chromosome_time_windows = chromosome[:time_window_length]
        first_part_time = chromosome_time_windows[:point1]  # First part of time window
        second_part_time = chromosome_time_windows[point1:]  # Second part of time window
        # Concatenate them in swapped order to create a new time window section
        new_time_window_section = second_part_time + first_part_time

        # Swap mutation for asteroid assignments (from index 25 onward)
        asteroid_number = random.randint(1, n_asteroids - 1)
        point2 = (asteroid_number*3)


        # Perform the swap in asteroid assignment part without changing values
        chromosome_asteroid_assignments = chromosome[asteroid_assignments_start:]
        first_part_asteroid = chromosome_asteroid_assignments[:point2]
        second_part_asteroid = chromosome_asteroid_assignments[point2:]
        # Concatenate them in swapped order to create a new asteroid assignment section
        new_asteroid_assignment_section = second_part_asteroid + first_part_asteroid

        # Create the final mutated chromosome by concatenating the time windows and asteroid assignments
        mutated_chromosome = new_time_window_section + new_asteroid_assignment_section
        # Ensure no station ID becomes invalid after mutation
        for i in range(time_window_length, len(mutated_chromosome), 3):
            if i + 3 <= len(mutated_chromosome):
                asteroid_id, station_id, opportunity_id = mutated_chromosome[i:i + 3]
                if station_id < 1 or station_id > n_stations:
                    mutated_chromosome[i + 1] = random.randint(1, n_stations)  # Fix invalid station_id

        repair_activity_windows(mutated_chromosome, n_stations);
        return mutated_chromosome

    # If no mutation occurs, return the original chromosome
    return chromosome
...
def genetic_algorithm_delivery_schedule():      
    ...
    #exit()
    #exit()
    population = initialize_population(n_stations, 
                                       n_asteroids, 
                                       population_size, 
                                       ts)
    best_solution = None
    ...
    for generation in range(generations):
        ...
        fitness_values = [custom_fitness(individual, ts.flat_db) 
...
ts = trappist_schedule(path=os.path.join(os.path.dirname(__file__), 
                                         'data', 'spoc', 'scheduling', 
                                         'candidates.txt'))
print(solution)
ts.plot(solution, path="./images/RW_Swap_Mass.png")
ts.plot_time(x=solution, path="./images/RW_Swap_Time_Window.png")
ts.pretty(solution)