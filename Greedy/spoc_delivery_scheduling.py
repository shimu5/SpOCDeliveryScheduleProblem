...
class trappist_schedule:
        ...
        for ast_id, stations in db.items():
            for stat_id, opps in stations.items():
                ...
                for idx, opp in enumerate(opps):
                    flat_db[(ast_id, stat_id, idx)] = opp  #SANote:Perhaps need to add a +1 to the idx value
                    #print((ast_id, stat_id, idx+1)); print(":"); print(opp)
    ...
    def _fitness_impl(
        ...
        valid_triplets = [t for t in triplets if t in self.flat_db]
        ...
        if len(gaps) > 0:
            ...
            print("Warning: No gaps found between station windows.")
            ineq_constraints.append(0)  # Append a default value, like 0, if no gaps exist
        ...
        # Ensure arrival_times and station_window_start_times have the same length ## Added by Shamima
        if len(arrival_times) != len(station_window_start_times):
            print("Warning: Mismatch in array lengths. Adjusting to match sizes.")
            min_length = min(len(arrival_times), len(station_window_start_times))
            arrival_times = arrival_times[:min_length]
            station_window_start_times = station_window_start_times[:min_length]
            station_window_end_times = station_window_end_times[:min_length]    #End Added by Shamima
        ...
        if plotting:
            ...
            for triplet in valid_triplets:
                if triplet[1] > 0:
                    # Check if the triplet exists in self.flat_db before accessing it
                    if triplet in self.flat_db:
                        schedule[triplet[1]]["assignments"].append(self.flat_db[triplet][0])
                    else:
                        print(f"Warning: triplet {triplet} not found in self.flat_db. Appending default value.")
                        schedule[triplet[1]]["assignments"].append(0)  # Appending default value like 0
    ...
    def example(self):
        ...
        assignments = {}
        # Iterate over every asteroid ID in a sorted order (sequential)
        for ast_id in sorted(self.db.keys()):
            print("asteroid_id - ", ast_id)
            
            # Create a set of station IDs.
            # stations = set(self.db[ast_id].keys()).difference(set(assignments.keys()))
            stations = set(self.db[ast_id].keys()) #sa
            
            while len(stations) > 0:
                stat_id = stations.pop()

                # Check if the asteroid / station pair is already assigned.
                #if stat_id in assignments: #SA
                #    continue

                opps = self.db[ast_id][stat_id]
                #print("opportunities", opps); break
                ...
                for opp_idx, opp in enumerate(opps, 1):
                    ...
                    for _, vals in assignments.items():
                        if ast_id == vals[0] or (vals[2] - 2 * self.station_gap) <= opp[0] <= (vals[3] + 2 * self.station_gap):
                            conflict = True
                            break
                    if not conflict:
                        assignments[stat_id] = [
                        ...
                        print("station_id", stat_id, assignments[stat_id])
                        break
